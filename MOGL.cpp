...
const GLchar *FragmentShader = GLSL(440,
		...
		out vec4 vertex_Color; //vec4 variable that will reference the vertex colors passed into the framgent shader from the vertex shader
		void main(){
...
void URenderGraphics(void)
{
	...
	GLuint totalVerticies = 6; //Specifies the number of the verticies for the triangle i.e.
	...
	glutSwapBuffers(); //Flips the back buffer with the front buffer every fram. Similar to GL Flush
...
void UCreateVBO(void)
{
	...
	GLfloat verts[] =
	{
			...
			/*index 3*/
			0.0f, 0.0f, //top-center of the screen
			1.0f, 0.0f, 0.0f, 1.0f, // Red vertex
	...
	/*Sets an attribute pointer position for the vertex colors i.e. Attribute 1 from rgba floats. Attribute 0 was for position x, y
	 */
