...
const GLchar *cubeFragmentShaderSource = GLSL(330,
		...
		void main(){
			...
			/*Phong lighting model calculations to generate ambient, diffuse, and specular components */
			//Calculate Ambient lighting for light source 2
			float ambientStrength = 0.1f; //Set ambient or global lighting strength
			vec3 ambient1 = ambientStrenght * lightColor1; //Generate ambient light color

			//Calculate Diffuse lighting for light source 2
			vec3 norm = normalize(Normal); //Normalize vectors to 1 unit
			vec3 lightDirection1 = normalize(lightPos1-FragmentPos); //Calculate distance (light direction)between light source and fragment/pixels on
			float impact = max(dot(norm, lightDirection1), 0.0); //Calculate diffues impact by generating dot product of normal and light
			vec3 diffuse1 = impact * lightColor1; //Generate diffuse light color

			//Calculate Specular lighting for light source 2
			float specularIntensity1 = 0.8f; //Set specular light strength
			float highLightSize1 = 16.0f; // Set specular highlight size
			vec3 viewDir = normalize(viewPosition - FragmentPos); //Calculate view direction
			vec3 reflectDir = reflect(-lightDirection, norm); //Calculate reflection vector

			//Calculate specular component for light source 2
			float specularComponent = pow(max(dot(viewDir, reflectDir), 0.0), highlightSize);
			vec3 specular1 = specularIntensity1 * specularComponent * lightColor1;




			//Calculate phong result
			vec3 phong = (ambient1 + diffuse1 + specular1) * objectColor;
			cubeColor = vec4(phong, 1.0f); //Send lighting results to GPU
}
);
...
void URenderGraphics(void)
{
	...
	GLint modelLoc, viewLoc, projLoc, objectColorLoc, lightColorLoc, lightPositionLoc, lightPositionLoc1, viewPositionLoc;
	...
	objectColorLoc = glGetUniformLocation(cubeShaderProgram, "objectColor");
	lightColorLoc = glGetUniformLocation(cubeShaderProgram, "lightColor");
	lightPositionLoc1 = glGetUniformLocation(cubeShaderProgram, "lightPos1");
	viewPositionLoc = glGetUniformLocation(cubeShaderProgram, "viewPosition1");

	//Pass color, light, and camera data to the cube shader program's corresponding uniforms
	glUniform3f(objectColorLoc, objectColor.r, objectColor.g, objectColor.b);
	glUniform3f(lightColorLoc, lightColor1.r, lightColor1.g, lightColor1.b);
	glUniform3f(lightPositionLoc1, lightPosition1.x, lightPosition1.y, lightPosition1.z);
	glUniform3f(viewPositionLoc, cameraPosition.x, cameraPosition.y, cameraPosition.z);
	...
	//Use the light Shader and activate the lamp vertex array object for rendering and transorming
	    ...
	    model = glm::scale(model, lightScale1);
