...
#define WINDOW_TITLE "Assignment 4-1-3" // Macro for window title
...
void URenderGraphics(void)
{
	...
	//Transforms the objet
	glm::mat4 model;
	model = glm::translate(model, glm::vec3(0.0f, 0.0f, 0.0f)); // Place the object at the center of the viewport
	model = glm::rotate(model, 15.0f, glm::vec3(1.0f, 0.0f, 0.0f)); //Rotate the object 15 degrees on the X axis
