...
const GLchar *cubefragmentShaderSource = GLSL(330,
		...
		void main(){
			...
			vec3 lightDirection = normalize(lightPos - FragmentPos); //Calculate distance (light direction)between light source and fragment/pixels on
...
void URenderGraphics(void)
{
	...
	GLint modelLoc, viewLoc, projLoc, objectColorLoc, lightColorLoc, lightPositionLoc, viewPositionLoc, cameraRotation;
...
{
	//Cube Vertex Shader
	GLint cubeVertexShaderSource = glCreateShader(GL_VERTEX_SHADER); //Creates the vertex Shader
	...
	//Lamp Vertex Shader
	GLint lampVertexShaderSource = glCreateShader(GL_VERTEX_SHADER); //Creates the vertex Shader
